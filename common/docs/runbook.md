...
- Consumer resources such as Agent Runtime, consumer-specific Agent Registry
  Service, MCP server, GKE, Cloud Run, Artifact Registry, Runtime-specific
  endpoint IAM, and endpoint authorization policy are out of scope. Common-owned
  shared Registry Services are in scope.
...
- `google_agent_registry_service.*` for the five common domains documented in
  `README.md`
- Runtime-specific `google_iap_agent_registry_endpoint_iam_member` resources are
  consumer-owned and must not appear in a new common plan. Existing legacy
  bindings remain until separately migrated and reviewed.
