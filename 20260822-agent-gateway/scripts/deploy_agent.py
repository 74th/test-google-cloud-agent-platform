...
def build_config(args: argparse.Namespace) -> dict[str, object]:
    ...
    identity_type = getattr(args, "identity_type", "AGENT_IDENTITY")
    service_account = getattr(args, "service_account", None) if identity_type == "SERVICE_ACCOUNT" else None
    return {
...
def redacted_create_preview(config: dict[str, object]) -> dict[str, object]:
    """Return the exact non-secret REST payload used for dry-run review."""
    return build_rest_create_payload(config)
...
def main() -> None:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Print the non-secret create payload without cloud writes")
    ...
    try:
        ...
        if args.dry_run:
            print(json.dumps(redacted_create_preview(config), ensure_ascii=False, indent=2))
            return
        import google.auth
        ...
        if not isinstance(resource_name, str):
            raise RuntimeError("Runtime create response did not include a resource name")
        live = AuthorizedSession(credentials).get(
            f"https://{args.location}-aiplatform.googleapis.com/v1/{resource_name}", timeout=30
        ).json()
